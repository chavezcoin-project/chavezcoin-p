...
{
    // Wait for IRC to get it first - disabled with arepacoin
...
// testnet dns seed begins with 't', all else are arepacoin dns seeds.
static const char *strDNSSeed[][2] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real Arepacoin offering, you will want to remove the "t" that
    // appears before the node name and IP
    {"Arepacoin seed_3", "192.189.25.192"},
    {"Arepacoin seed_4", "192.189.25.191"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // arepacoin enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Arepacoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // arepacoin: mint proof-of-stake blocks in the background
