...
extern Value exportarepacoinkeys(const Array& params, bool fHelp);
...
// arepacoin: get network Gh/s estimate
// arepacoin note: this is only useful during the initial proof-of-work 'IPO' phase of the network
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Arepacoin address");
...
Value getarepacoinaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getarepacoinaddresses <account>\n"
            "Returns the list of addresses and the associated Arepacoin address for the given account.");
    ...
    {
        const CBitcoinAddress& address = item.first;
        const CArepacoinAddress arepacoinAddress(address);
        const string& strName = item.second;
        if (strName == strAccount)
            ret.push_back(Pair(address.ToString(), arepacoinAddress.ToString()));
    }
    return ret;
}
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Arepacoin address:")+s.name_);
...
Value distribute(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "amount is the the number of arepacoins to distribute, in double-precision floating point number.\n"
            "If proceed is not true the arepacoins are not sent and the details of the distribution are returned.");
    ...
    else
    {
        ...
        {
            Object obj;
            obj.push_back(Pair("arepacoin_address", distribution.GetPeershareAddress().ToString()));
            obj.push_back(Pair("balance", (double)distribution.GetBalance() / COIN));
            obj.push_back(Pair("arepacoin_address", distribution.GetArepacoinAddress().ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // arepacoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Portfolio encrypted; Arepacoin server stopping. Please restart server to run with encrypted portfolio";
}
...
// arepacoin: get information of sync-checkpoint
...
// arepacoin: reserve balance from being staked for network protection
...
// arepacoin: check wallet integrity
...
// arepacoin: make a public-private key pair
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "getarepacoinaddresses",   &getarepacoinaddresses,   true },
    ...
    { "exportarepacoinkeys",     &exportarepacoinkeys,     false },
...
std::string CallArepacoinRPC(const std::string &strMethod, const Array &params)
{
    if (mapArepacoinArgs["-rpcuser"] == "" && mapArepacoinArgs["-rpcpassword"] == "")
        throw runtime_error(strprintf(
            _("You must set rpcpassword=<password> in the Arepacoin configuration file:\n%s\n"
              "If the file does not exist, create it with owner-readable-only file permissions."),
                GetArepacoinConfigFile().string().c_str()));
    ...
    SSLIOStreamDevice d(sslStream, fUseSSL);
    iostreams::stream<SSLIOStreamDevice> stream(d);
    if (!d.connect(GetArepacoinArg("-rpcconnect", "127.0.0.1"), GetArepacoinArg("-rpcport", CBigNum(fTestNet? AREPACOIN_TESTNET_RPC_PORT : AREPACOIN_RPC_PORT).ToString().c_str())))
        throw runtime_error("couldn't connect to Arepacoin RPC server");

    // HTTP basic authentication
    string strUserPass64 = EncodeBase64(mapArepacoinArgs["-rpcuser"] + ":" + mapArepacoinArgs["-rpcpassword"]);
    map<string, string> mapRequestHeaders;
    mapRequestHeaders["Authorization"] = string("Basic ") + strUserPass64;
    ...
    if (error.type() != null_type)
    {
        ...
        throw arepacoin_rpc_error(nCode, sMessage);
    }
    else
    {
        // Result
