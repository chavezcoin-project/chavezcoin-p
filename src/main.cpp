...
// Copyright (c) 2011-2013 The Arepacoin developers
// Copyright (c) 2013-2014 The Arepacoin developers
...
// Global state
//
static const uint256 hashGenesisBlockOfficial("0x000004495bf6a0accdb12568b4a0f50d6d2779ccbdacc6807ad9c4a83d22daf4");
static const uint256 hashGenesisBlockTestNet ("0x00000fce3144c70dc8862699f8ae3ac89868bece10ff97934334b57c03028e4e");
...
int nCoinbaseMaturity = COINBASE_MATURITY_ARC;
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // Arepacoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Arepacoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // arepacoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// arepacoin: find block wanted by given orphan block
...
// arepacoin: minter's coin stake is rewarded based on coin age spent (coin-days)
...
// arepacoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // arepacoin: target change every block
    // arepacoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // arepacoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // arepacoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // arepacoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // arepacoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // arepacoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // arepacoin: track money supply and mint amount info
    ...
    // arepacoin: fees are not collected by miners as in bitcoin
    // arepacoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// arepacoin: total coin age spent in transaction, in the unit of coin-days.
...
// arepacoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // arepacoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // arepacoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // arepacoin: record proof-of-stake hash value
    ...
    // arepacoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // arepacoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // arepacoin: coinbase output should be empty if proof-of-stake block
    ...
    // arepacoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Arepacoin: check switch from proof of work to proof of stake
    ...
    // arepacoin: check that the block satisfies synchronized checkpoint
    ...
    // arepacoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // arepacoin: check proof-of-stake
    ...
    // arepacoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // arepacoin: ask for missing blocks
            if (pfrom)
    ...
    // arepacoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // arepacoin: getblocks may not obtain the ancestor block rejected
    ...
    // arepacoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Arepacoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Arepacoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis Block:
        // CBlock(hash=000004495bf6a0accdb1, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=c9acf878c4, nTime=1505792691, nBits=1e0fffff, nNonce=384202, vtx=1, vchBlockSig=)
        //   Coinbase(hash=c9acf878c4, nTime=1505792691, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //     CTxIn(COutPoint(0000000000, -1), coinbase 04ffff001d020f2745546865206d6f73742064656c6963696f757320636f696e207468617420686173206576657220686164207468652063727970746f63757272656e63696573202d2032303137)
        //     CTxOut(empty)
        //   vMerkleTree: c9acf878c4

        // Genesis block
        const char* pszTimestamp = "The most delicious coin that has ever had the cryptocurrencies - 2017";
        unsigned int nTimeGenesis=1505792691;
        unsigned int nNonceGenesis=384202;

        if (fTestNet)
        {
            pszTimestamp="Russia and China Declare All Out War on US Petrodollar - July 16, 2017";
            nTimeGenesis=1496491392;
            nNonceGenesis=640954;
        }
        ...
        printf("Arepacoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();

        printf("End Arepacoin Genesis Block\n");
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xc9acf878c405f3eb7aa96216b871a3c25db7265a76c185b418c0e1366e90501b"));
        else
            assert(block.hashMerkleRoot == uint256("0x08b31ec7aabba034f517c207dfe5888d646ee74f6319bd11ebf2449e63e0cb13"));
        ...
        // arepacoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // arepacoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // arepacoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // arepacoin: should not enter safe mode for longer invalid chain
    // arepacoin: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 10) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = "WARNING: Checkpoint is too old. Please wait for the block chain to download, or notify developers of the issue (https://github.com/Arepacoin/Arepacoin/issues).";
    }

    // arepacoin: if detected invalid checkpoint enter safe mode
    if (Checkpoints::hashInvalidCheckpoint != 0)
    {
        nPriority = 3000;
        strStatusBar = strRPC = "WARNING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers of the issue (https://github.com/Arepacoin/Arepacoin/issues).";
    }
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // arepacoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // arepacoin: record my external IP reported by peer
        ...
        // arepacoin: relay sync-checkpoint
        {
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // arepacoin: immediate retry during initial download
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // arepacoin: send latest proof-of-work block to allow the
                        // download node to accept as orphan (proof-of-stake 
                        // block might be rejected by stake connection check)
                        // arepacoin: send latest block
                        vector<CInv> vInv;
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // arepacoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // arepacoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // arepacoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // arepacoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
