...
// arepacoin: optional setting to unlock wallet for block minting only;
...
// arepacoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // arepacoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // arepacoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar", true)) // arepacoin: not avatar mode; arepacoin: avatar mode enabled by default to avoid change being sent to hidden address
                    {
...
// arepacoin: create coin stake transaction
...
// arepacoin: check 'spent' consistency between wallet and txindex
// arepacoin: fix wallet spent state according to txindex
...
// arepacoin: disable transaction (only for coinstake)
...
void CWallet::ExportArepacoinKeys(int &nExportedCount, int &nErrorCount)
{
    nExportedCount = 0;
    nErrorCount = 0;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, mapAddressBook)
    {
        ...
        if (address.IsScript())
        {
            ...
            json_spirit::Array vArepacoinAddressStrings;
            BOOST_FOREACH(const CBitcoinAddress &address, vAddresses)
                vArepacoinAddressStrings.push_back(CArepacoinAddress(address).ToString());

            json_spirit::Array params;
            params.push_back(json_spirit::Value(nRequired));
            params.push_back(vArepacoinAddressStrings);
            params.push_back("Arepacoin");

            try
            {
                string result = CallArepacoinRPC("addmultisigaddress", params);
                printf("Exported multisig address %s: %s\n", address.ToString().c_str(), result.c_str());
                nExportedCount++;
            }
            catch (arepacoin_rpc_error &error)
            {
        ...
        else
        {
            ...
            json_spirit::Array params;
            params.push_back(CArepacoinSecret(vchSecret, fCompressed).ToString());
            params.push_back("Arepacoin");
            try
            {
                string result = CallArepacoinRPC("importprivkey", params);
