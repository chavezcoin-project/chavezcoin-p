...
void DividendDistributor::GenerateOutputs(int nTransactions, vector<Object> &vTransactionOuts) const
{
    ...
    BOOST_FOREACH(const Distribution &distribution, vDistribution)
    {
        ...
        string address = distribution.GetArepacoinAddress().ToString();
...
DividendDistributor GenerateDistribution(const BalanceMap &mapBalance, double dAmount)
{
    ...
    printf("Distributing %f arepacoins to %d addresses with a minimum payout of %f\n", dAmount, mapBalance.size(), dMinPayout);
...
{
    // As of 2014-02-22, Arepacoin won't generate transactions larger than 100,000 bytes (MAX_BLOCK_SIZE_GEN/5)
...
Array SendDistribution(const DividendDistributor &distributor)
{
    try {
        ...
        if (dTotalDistributed > dBalance)
            throw runtime_error("Not enough arepacoins available in distribution account");
        ...
        printf("Will send %f arepacoins to %d addresses in %d transactions\n", dTotalDistributed, nDistributionCount, nTransactions);
        ...
        BOOST_FOREACH(Object &output, vOutputs)
        {
            ...
            result = CallArepacoinRPC("sendmany", sendmanyParams);
...
double GetDistributionBalance()
{
    ...
    return boost::lexical_cast<double>(Value(CallArepacoinRPC("getbalance", params)).get_str());
}
