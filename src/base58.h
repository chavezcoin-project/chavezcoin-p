...
class CArepacoinAddress : public CBitcoinAddress
{
public:
    enum
    {
        PUBKEY_ADDRESS = 55,  // Arepacoin: addresses begin with 'P'
        SCRIPT_ADDRESS = 117, // Arepacoin: addresses begin with 'p'
    ...
    CArepacoinAddress(uint160 hash160In)
    {
        SetHash160(hash160In);
    }

    CArepacoinAddress(const CBitcoinAddress &address)
    {
        if (address.IsScript())
...
class CArepacoinSecret : public CBitcoinSecret
{
public:
    void SetSecret(const CSecret& vchSecret, bool fCompressed)
    {
        assert(vchSecret.size() == 32);
        SetData(128 + (fTestNet ? CArepacoinAddress::PUBKEY_ADDRESS_TEST : CArepacoinAddress::PUBKEY_ADDRESS), &vchSecret[0], vchSecret.size());
        if (fCompressed)
            vchData.push_back(1);
    }
    ...
    bool IsValid() const
    {
        ...
        switch(nVersion)
        {
             case (128 + CArepacoinAddress::PUBKEY_ADDRESS):
                break;

            case (128 + CArepacoinAddress::PUBKEY_ADDRESS_TEST):
                fExpectTestNet = true;
                break;
    ...
    CArepacoinSecret(const CSecret& vchSecret, bool fCompressed)
    {
        SetSecret(vchSecret, fCompressed);
    }
};
